(() => {
  const documents = {
    home: `
      <div class="prd-summary"><span>页面属性<b>存量首页增量改造</b></span><span>本次新增<b>从题库 / 知识库添加</b></span><span>结果承接<b>当前对话</b></span></div>
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>新增位置</th><th>新增内容</th><th>结果</th></tr></thead><tbody>
          ...
          <tr><td>首页“＋”菜单 → 从知识库添加</td><td>打开独立的小型知识库浮层，不复用全屏题库选择弹窗。</td><td>可浏览并勾选个人资料或题单，加入当前对话。</td></tr>
          ...
          <tr><td>题库选择弹窗</td><td>新增教材章节/知识点选题、题型、难度、搜索、答、AI改编、添加及积分解锁（仅平台题库，不提供「我的题库」切换）。</td><td>作为首页覆盖层出现，不改变原页面路由。</td></tr>
        </tbody></table></div>
        <h4>新增功能主流程</h4>
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>控件</th><th>触发与结果</th><th>状态/约束</th></tr></thead><tbody>
          ...
          <tr><td>“从知识库添加”</td><td>在“＋”菜单附近打开紧凑浮层，提供搜索、文件夹展开与多选。</td><td>不跳转“我的知识库”页面；从题库添加仍使用原大弹窗。</td></tr>
      ...
      <section class="prd-section">
        <h3>2.1 从知识库添加 <span class="prd-status">P0</span></h3>
        <ul>
          <li>入口：首页对话输入区直接点击“＋”并选择<strong>从知识库添加</strong>；AI组题就绪态中的同名入口复用相同行为。</li>
          <li>使用靠近“＋”菜单的<strong>小型浮层</strong>，不使用全屏遮罩，不改变页面路由；浮层包含搜索、已选数量、取消与加入对话。</li>
          <li>系统文件夹顺序固定为：<strong>我的工作成果 → 我的题单 → 对话中上传的文件</strong>。</li>
          <li>“我的题单”读取<strong>我的知识库 → 我的云盘 → 我的题单</strong>中的个人已保存题单；支持展开/收起，题单条目展示名称、题量与更新时间。</li>
          <li>支持跨文件夹多选；顶部显示“已选 N 项”，N=0 时禁用加入对话。确认后资料以附件形式进入当前对话，用户仍停留首页。</li>
          <li>搜索覆盖文件夹内资料与个人题单名称；无结果时保留文件夹结构并展示空状态。</li>
        </ul>
      </section>
      <section class="prd-section">
        <h3>3. 从题库中添加：结构与筛选</h3>
        <ul>
          <li>弹窗仍覆盖在当前首页，不跳转飞象题库。左侧保留学段学科；选题方式与飞象题库「题库选题」一致：子页签「教材章节｜知识点」（不提供「我的题库」入口）。</li>
          <li>教材章节：两列教材版本选择器 + 章节树（人教版四年级上册默认展开「一 万以上数的认识」）；知识点：知识点树，无教材选择器、无「全部知识点」、树节点<strong>不</strong>展示题量。</li>
          <li>学段学科与教材版本与飞象题库共用本地记忆（同一老师上下文）。右侧：题型、难度、关键词搜索、题目卡片、分页或解锁提示。</li>
          <li>切换学科、题库来源、章节/知识点、题型、难度或关键词后回到第1页；已选题保持，除非题目已失效。</li>
        </ul>
      </section>
      <section class="prd-section">
      ...
      <section class="prd-section"><h3>7. 首页验收场景</h3><ul><li>分别验证首页＋、AI组题后＋均可进入题库与知识库选择器。</li><li>知识库选择器验证小型浮层尺寸、文件夹顺序、“我的题单”展开、搜索、跨文件夹多选、取消及加入对话。</li><li>分别构造0、7、20、21、40、41、60、61题数据检查题库选择器底部状态。</li><li>验证余额不足、重复确认、接口超时、跨页选题、筛选后返回、取消和加入对话。</li><li>加入完成后仍在首页，题库选择正确显示“已选N道题”，知识库选择正确显示“已选N项”。</li></ul></section>
    ...
    teachingCompose: `
      ...
      <section class="prd-section">
        ...
        <p>用户从 AI 组题任务进入<strong>飞象题库</strong>（或直接在 Demo 工作台组题）后，题库选题、组题工具、右侧画布、自动保存、下载等<strong>一律以《飞象题库》PRD 为准</strong>，本 Tab 不重复展开：</p>
    ...
    workbench: `
      ...
      <section class="prd-section">
        ...
        <p>飞象题库为新增页面。左侧负责找题、预览题源与 AI 组题过程，右侧为当前题单画布。左侧顶部为<strong>来源页签栏</strong>：返回、飞象象标（无「飞象题库」文字）、学段学科下拉，固定「飞象题库」「我的题库」「组题工具」，以及由组题工具衍生的<strong>可关闭动态页签</strong>（规则见 §2）。</p>
        <ul><li>飞象题库（原「题库选题」）：下分「教材章节」「知识点」。「教材章节」与飞象老师「作业」一致：选学段学科、教材版本后按章节树选题。「知识点」按学科知识树浏览，不选教材版本。</li><li>组题工具：上传文件、AI组题（已保存题单改从「我的题库 → 题单」选用）。</li><li>我的题库：子页签「试题｜题单」。试题为个人入库题目（上传/录题），支持知识点/题型/难度/搜索，树节点展示题量；题单为知识库「我的题单」列表，可查看并整份/逐题加入当前画布。</li><li>右侧画布：展示已选题目、答案解析及作答区；须点击<strong>保存</strong>才写入「我的题单」。</li></ul>
      </section>
      <section class="prd-section">
        <h3>2. 左侧来源页签与动态 Tab <span class="prd-status">P0</span></h3>
        <p>用户在「组题工具」内进入具体能力，或继续「查看」某条上传记录 / 知识库题单 / AI 组题对话时，在「组题工具」<strong>右侧依次追加</strong>动态页签。切换页签只改变<strong>左侧内容区</strong>，右侧当前题单始终保留。</p>
        <h4>页签类型</h4>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>类型</th><th>示例</th><th>关闭</th></tr></thead><tbody>
          <tr><td>固定页签</td><td>飞象题库、我的题库、组题工具</td><td><strong>不可</strong>关闭；两个题库入口相邻，组题工具置后；始终位于栏最左侧（飞象象标与学段学科下拉之后）。</td></tr>
          <tr><td>动态页签 · 入口</td><td>上传文件、AI组题</td><td>右侧 <strong>×</strong> 可关闭；同一入口全局<strong>最多一个</strong>页签，重复进入则激活已有页签。</td></tr>
        ...
        <h4>打开顺序</h4>
        <ul>
          <li>动态页签按<strong>打开时间</strong>从左到右排列在「组题工具」之后，先开的在左、后开的在右。</li>
          <li>点击页签标题切换激活态；激活页签下方绿色指示条与 Demo 一致。</li>
          <li>从「组题工具」卡片进入「上传文件 / AI组题」时，若对应入口页签不存在则新建，若已存在则<strong>仅切换激活</strong>。</li>
          <li>在入口页内点击「查看并选题」「查看题单」「查看历史对话」等，为当前上下文<strong>再开一层</strong>子页动态页签，标题取文件名（去后缀）、题单标题或对话摘要（过长省略）。</li>
        </ul>
        <h4>关闭与激活回退（对齐浏览器标签页）</h4>
        <ul>
          <li>点击动态页签上的 <strong>×</strong> 关闭该页签，并释放其左侧视图状态（未保存的纯浏览状态可丢弃；右侧画布已加题不受影响）。</li>
          <li>若关闭的是<strong>当前激活</strong>页签：激活<strong>左侧相邻</strong>页签（与 Chrome 关闭当前标签一致）；若其左侧无动态页签，则激活「组题工具」。</li>
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>入口</th><th>打开结果</th><th>返回</th></tr></thead><tbody>
          <tr><td>首页明确进入工作台</td><td>新建或恢复最近草稿；左侧默认飞象题库（教材章节）。学段学科、教材版本恢复该老师上次选择；首次进入用学科默认版本。</td><td>退出回首页，先保存草稿。</td></tr>
          ...
          <tr><td>历史AI组题/上传记录</td><td>左侧打开对应记录页；右侧保留当前画布。</td><td>关闭对应动态页签（§2）；必要时回到「组题工具」或「飞象题库」。</td></tr>
        </tbody></table></div>
      </section>
      <section class="prd-section">
        <h3>4. 飞象题库：教材章节、知识点与选题范围 <span class="prd-status">P0</span></h3>
        <p>「飞象题库」左侧为<strong>教材章节 / 知识点</strong>。「教材章节」与作业一致：选教材版本后按章节树选题。「知识点」按学科上线知识树选题，不展示教材版本。学段学科、教材版本写入本地（按老师记忆），下次进入飞象题库仍恢复上次选择；每个学科分别记住其教材版本。树数据来自平台题库已上线知识树，不在前端写死 Demo 树。</p>
        <h4>开放学科（本期）</h4>
      ...
      <section class="prd-section">
        ...
        <p>平台题库严格复用首页题库的数量、积分和分页规则：每页 20 题、一次消耗 20 积分、最多 3 页 / 60 题。不得在两个入口实现两套口径。</p>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>数量</th><th>工作台行为</th><th>验收重点</th></tr></thead><tbody>
          ...
          <tr><td>&gt;20</td><td>首屏 20 题；底部按实际剩余题量提示「还有 N 道题，消耗 20 积分可解锁」；解锁后显示页码。</td><td>题量与后续页数必须动态计算，不得固定写「后 2 页、共 60 题」；上限 60 题。</td></tr>
        </tbody></table></div>
        <p>筛选变化回第1页并重置飞象分页至第 1 页；解锁权益按<strong>自然日</strong>读取服务端；翻页不清空已选或右侧画布。「我的题库 → 试题」不走积分解锁；树节点<strong>展示题量</strong>；试题页签左对齐展示<strong>共 N 题</strong>（N=个人入库题目总数）。「题单」页签展示已保存题单列表，交互同原「复用我的题单」。</p>
        <p>题库搜索框内容需持久化到本地（如 <code>localStorage</code>），用户切换到「组题工具」等其它左侧页签再返回「飞象题库」或「我的题库 → 试题」时，搜索词与筛选结果状态保持一致；切换学段学科或顶栏「飞象题库 / 我的题库」时，分别恢复该维度下上次保存的搜索词。</p>
      </section>
      <section class="prd-section">
        <h3>6. 空状态与提示文案</h3>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>场景</th><th>展示</th><th>操作</th></tr></thead><tbody>
          ...
          <tr><td>飞象 · 有题但被筛选/搜索筛空</td><td>只展示主提示「未找到符合条件的题目」，不同时展示知识点无结果提示。</td><td>提供「清除筛选」，点击后清空关键词、题型、难度和知识点筛选。</td></tr>
          <tr><td>我的题库 · 当前学科下无任何个人题（total=0）</td><td>中间区域居中展示：标题<strong>还没有题目</strong>；说明<strong>上传题目和答案文件，AI 识别并打标，自动存入我的题库后即可在此选用</strong>；主按钮<strong>上传文件</strong>（带上传图标）。</td><td>点击按钮进入「组题工具 → 上传文件」页（Demo 直达上传页）；上传完成后题目进入我的题库。</td></tr>
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>步骤</th><th>交互</th><th>规则</th></tr></thead><tbody>
          ...
          <tr><td>3.1 生成中 · 生成进度</td><td>结果未返回前，在改编区内展示<strong>生成进度</strong>（分步文案 + loading）；提示「生成完成后自动收起」。</td><td>仅生成中可见；<strong>改编结果就绪后生成进度自动隐藏</strong>，不得与结果列表并存。</td></tr>
          <tr><td>4. 改编结果</td><td>卡片内列表展示 2–3 道改编结果；每道<strong>不展示任何标签</strong>（无题型/难度/知识点角标），仅保留「AI 改编 N」序号与题干；右侧操作为<strong>「＋」图标</strong>（title/aria 仍为「选用题目」）。</td><td>点击「＋」后才加入右侧画布或首页已选；未选用关闭面板不影响原题。</td></tr>
      ...
      <section class="prd-section">
        <h3>8. 组题工具</h3>
        <p>页面标题下不展示总括性说明，直接呈现三张入口卡片。第一张标题<strong>上传文件</strong>，说明<strong>上传题目和答案文件，AI 识别并打标，自动存入我的题库</strong>；第二张标题<strong>复用我的题单</strong>，说明<strong>选择已保存的题单，支持整份或逐题加入当前题单</strong>。</p>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>入口</th><th>产品定义</th><th>完成态</th></tr></thead><tbody>
          <tr><td>上传文件</td><td>卡片文案：<strong>上传题目和答案文件，AI 识别并打标，自动存入我的题库</strong>。上传区主文案为「从电脑选择文件，或把文件拖到这里」。AI 识别、拆题、提取答案并自动打标，写入「我的题库」。</td><td>每份上传任务独立进入进度列表；<strong>解析成功后</strong>题目以<strong>结构化题目</strong>写入「我的题库」（见 §9.1），可逐题或全部加入当前题单。</td></tr>
          <tr><td>复用我的题单</td><td>读取<strong>我的知识库 → 我的云盘 → 我的题单</strong>文件夹下的题单（画布<strong>手动保存</strong>等），支持预览、逐题选用、全部选用。</td><td>题目加入当前画布；原题单不变。</td></tr>
          <tr><td>AI组题</td><td>根据题量、知识点、题型、难度和时长生成完整候选题单；输入区支持<strong>文字、附件（＋）、语音</strong>。</td><td>支持逐题/全部加入当前画布；<strong>不</strong>写入「我的云盘 → 我的题单」。</td></tr>
        </tbody></table></div>
        <h4>8.1 AI组题 · 输入区</h4>
        <ul>
          <li>入口：<strong>组题工具 → AI组题</strong>（或「开始组题」进入同一页）。</li>
        ...
        <h4>8.2 AI录题 · AI录题进度</h4>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>状态</th><th>列表展示</th><th>操作</th></tr></thead><tbody>
          <tr><td>解析中</td><td>状态标签「解析中」、文件名、提交时间、当前阶段文案。</td><td><strong>不展示</strong>「查看进度」等按钮；用户可离开页面，后台继续解析。</td></tr>
          <tr><td>解析完成</td><td>「解析完成」、题量、完成时间；入库反馈统一使用<strong>「已保存到“我的题库”」</strong>。</td><td>「查看并选题」进入题目列表；列表内每题为<strong>结构化题目</strong>（与平台/我的题库卡片一致），可逐题或全部选用加入当前题单。</td></tr>
          <tr><td>解析失败</td><td>「解析失败」、失败原因摘要。</td><td>展示<strong>「重新解析」</strong>；点击后任务回到解析中并重新走解析链路，不重复创建个人题库脏数据。</td></tr>
          <tr><td>无任何上传任务</td><td>「AI录题进度」列表区域展示空状态文案<strong>暂无记录</strong>（不展示表头占位行）。</td><td>保留上传区，引导用户上传首张图片或文件。</td></tr>
        </tbody></table></div>
        <h4>8.3 我的题单</h4>
        <p>入口：<strong>组题工具 → 复用我的题单</strong>。页面说明统一为<strong>「这里展示你已保存的题单，可整份或逐题加入当前题单。」</strong>；右侧当前题单保持可见。</p>
        <h4>数据来源与列表</h4>
        ...
        <ul>
          ...
          <li>列表页展示：题单<strong>标题</strong>、<strong>元信息</strong>（统一为「N 题 · M月D日更新」，不展示「自动保存」，不使用 04-12 等数字日期）、操作<strong>「查看」</strong>；列表顶部或说明区固定展示路径「我的知识库 / 我的云盘 / 我的题单」。预览详情只展示一份题量与更新时间，不重复题量。</li>
        ...
        <div class="prd-note warn">与知识库「编辑」的区别：「编辑」以 <code>paperId</code> 打开整份题单进入画布<strong>编辑原稿</strong>（切换规则见 §9.9）；「我的题单」是在<strong>当前正在组题的画布</strong>上追加题目，不改变知识库原题单内容。</div>
        <h4>异常与边界</h4>
        <ul>
      ...
      <section class="prd-section">
        ...
        <p>当前题单是题单的<strong>所见即所得编辑区</strong>：支持<strong>结构化</strong>与<strong>非结构化</strong>题目混排，拖拽排序，富文本与公式排版，作答区与导出。题目均经左侧来源（题库选题、组题工具）<strong>选用即进入画布</strong>；进入后在右侧改序、改字、改公式、删题。</p>
        ...
        <h4>9.4 题单排版工具（使用方式）</h4>
        <p>题单排版工具固定在画布上方、卷面滚动区之上，<strong>水平居中</strong>。作用范围如下：</p>
        ...
        <p>题单<strong>标题</strong>可在卷面直接编辑。<strong>学校 / 班级 / 姓名</strong>栏<strong>不在画布展示</strong>，仅在<strong>下载 Word</strong>时自动插入卷首（固定下划线占位，供打印后手写）。题目默认<strong>紧凑排列</strong>，不画整题分隔横线（除非用户插入）。</p>
        ...
        <ul>
          <li>公式与纯文本混排；导出 Word 时需保留公式渲染或降级策略（产品/export 联调定义）。</li>
        ...
        <ul>
          <li>题卡「答」：展开/收起<strong>答案与解析</strong>；默认收起；下载 Word 时题目、答案、解析合并在同一文件（见 §9.9）。</li>
        ...
        <ul>
          ...
          <li>顶栏按钮顺序：<strong>＋</strong> → <strong>保存</strong> → <strong>下载</strong>；新建／切换题单入口<strong>只显示加号，不展示「新建题单」文字</strong>。「我的题单」列表仅含曾保存的题单。</li>
          <li><strong>＋ 新建 · 空画布置灰</strong>：当前画布<strong>没有任何题目</strong>时，顶栏「＋」置灰不可点；tooltip「画布为空，请先添加题目」。</li>
          <li><strong>＋ 新建 · 未保存拦截</strong>：画布有题且相对最后一次保存有改动时，点击「＋」弹出提示「<strong>请先保存当前题单，以防数据丢失</strong>」；用户须<strong>保存成功</strong>后再次点击「＋」才允许新建空白题单或切回暂存题单（见下方切换规则）。弹窗提供「保存」与「取消」。</li>
          ...
          <li><strong>下载题单（本期唯一导出）</strong>：顶栏下载 icon → 确认弹窗 → 主按钮<strong>「下载 Word」</strong>；说明<strong>「将先保存当前题单，再下载 Word 文档」</strong>。题目、答案、解析合并在同一文件；卷首含学校/班级/姓名栏。以当前画布内容为准；若存在未保存改动，确认下载时自动执行保存。</li>
          <li><strong>本期不做</strong>：PDF 导出、学生版/教师版分册、多格式选择、题单分享链接等；需求与 Demo 均不得出现上述能力描述。</li>
          <li>保存失败：§10 提示并可重试；保存按钮不得置灰。</li>
        </ul>

        <div class="prd-note">Demo 已实现：题单排版工具、浮动富文本条、公式/符号模态框、删空即删题、未命名题单递增、<strong>Word 合并下载</strong>（卷首学籍栏）、画布<strong>手柄拖拽排序</strong>（§9.2）、<strong>保存按钮置灰/高亮 + 保存时间</strong>、<strong>＋ 空画布置灰 + 未保存弹窗拦截</strong>、<strong>下载前自动保存</strong>、<strong>云盘编辑切换丢弃未保存题单</strong>（§9.9）。<strong>表格模式公式、完整 LaTeX 渲染</strong>仍待对齐。</div>
      </section>
      <section class="prd-section">
        <h3>10. 工作台状态与异常</h3>
        <ul><li>空画布：标题「当前题单还没有题目」；说明「可以从题库选择，也可以通过AI录题、复用题单或AI组题添加题目。」</li><li>加载中：保留布局骨架，禁止重复加题；单个卡片失败不阻断列表。</li><li>保存失败：顶部提示“未保存”，支持重试；不得伪装成已保存。</li><li>并发编辑：本期至少用更新时间检测覆盖风险，冲突时提示保留本地或加载新版本。</li><li>题目失效：画布已有副本仍可编辑；来源处显示已下架，不允许再次添加。</li></ul>
      </section>`,
    ...
    knowledge: `
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>按钮</th><th>目标行为</th><th>验收规则</th></tr></thead><tbody>
          ...
          <tr><td>编辑</td><td>跳转飞象题库，以 <code>paperId</code> 载入，<strong>右侧当前题单直接打开该题单</strong>（非左侧预览）。若工作台当前画布另有题单且<strong>已有题目</strong>，先保留该题单编辑进度再切换；当前题单<strong>保存且无未保存改动</strong>、画布有题时，可通过画布 <strong>＋</strong> 切回（规则见工作台 §9.9.1）。</td><td>不得打开错误题单；进入前保存知识库列表状态。</td></tr>
      ...
      <section class="prd-section">
        <h3>6. 删除与权限边界</h3>
        <ul><li>删除使用软删除并支持撤销。</li><li>只有拥有者可编辑/删除。</li><li>下载失败保留预览页并支持重试；文件名使用题单标题并过滤非法字符。</li><li><strong>非本期</strong>：题单分享、外链只读、协作者权限、PDF/学生版/教师版导出（不得写入本期验收）。</li></ul>
      </section>
      <section class="prd-section"><h3>7. 知识库验收场景</h3><ul><li>新用户空目录；1条、多条、重名题单；搜索无结果。</li><li>画布点击保存后题单出现在列表；再次保存同步题数与保存时间；从未保存的题单不出现；仅 AI 组题不出现于此文件夹。</li><li>查看留在知识库；编辑进入工作台右侧指定画布；返回状态正确。</li><li>预览页/工作台下载均为 Word 合并版；删除取消/成功/失败/撤销。</li></ul></section>`
  }
