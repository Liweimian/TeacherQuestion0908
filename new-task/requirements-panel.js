  ...
  const documents = {
    ...
    workbench: `
      ...
      <section class="prd-section">
        ...
        <p>飞象题库为新增页面。左侧负责找题、预览题源与 AI 组题过程，右侧为当前题单画布。左侧顶部为<strong>来源页签栏</strong>：学段学科下拉，固定「题库选题」「组题工具」「我的题库」，以及由组题工具衍生的<strong>可关闭动态页签</strong>（规则见 §2）。</p>
        <ul><li>题库选题：下分「教材章节」「知识点」。「教材章节」与飞象老师「作业」一致：选学段学科、教材版本后按章节树选题，不另述交互。「知识点」按学科知识树浏览，不选教材版本。</li><li>组题工具：上传文件、复用我的题单、AI组题。</li><li>我的题库：个人入库题目，支持知识点/题型/难度/搜索；树节点展示题量。</li><li>右侧画布：展示已选题目、答案解析及作答区；须点击<strong>保存</strong>才写入「我的题单」。</li></ul>
      </section>
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>类型</th><th>示例</th><th>关闭</th></tr></thead><tbody>
          <tr><td>固定页签</td><td>题库选题、组题工具、我的题库</td><td><strong>不可</strong>关闭；始终位于栏最左侧（「飞象题库」标识与学段学科下拉之后）。</td></tr>
          <tr><td>动态页签 · 入口</td><td>上传文件、复用我的题单、AI组题</td><td>右侧 <strong>×</strong> 可关闭；同一入口全局<strong>最多一个</strong>页签，重复进入则激活已有页签。</td></tr>
        ...
        <ul>
          ...
          <li>从「组题工具」卡片进入「上传文件 / 复用我的题单 / AI组题」时，若对应入口页签不存在则新建，若已存在则<strong>仅切换激活</strong>。</li>
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>入口</th><th>打开结果</th><th>返回</th></tr></thead><tbody>
          <tr><td>首页明确进入工作台</td><td>新建或恢复最近草稿；左侧默认题库选题（教材章节）。学段学科、教材版本恢复该老师上次选择；首次进入用学科默认版本。</td><td>退出回首页，先保存草稿。</td></tr>
          ...
          <tr><td>历史AI组题/上传记录</td><td>左侧打开对应记录页；右侧保留当前画布。</td><td>关闭对应动态页签（§2）；必要时回到「组题工具」或「题库选题」。</td></tr>
        </tbody></table></div>
      </section>
      <section class="prd-section">
        <h3>4. 题库选题：教材章节、知识点与选题范围 <span class="prd-status">P0</span></h3>
        <p>「题库选题」左侧为<strong>教材章节 / 知识点</strong>。「教材章节」与作业一致：选教材版本后按章节树选题。「知识点」按学科上线知识树选题，不展示教材版本。学段学科、教材版本写入本地（按老师记忆），下次进入飞象题库仍恢复上次选择；每个学科分别记住其教材版本。树数据来自平台题库已上线知识树，不在前端写死 Demo 树。</p>
        <h4>开放学科（本期）</h4>
      ...
      <section class="prd-section">
        ...
        <p>筛选变化回第1页并重置飞象分页至第 1 页；解锁权益按<strong>自然日</strong>读取服务端；翻页不清空已选或右侧画布。「我的题库」不走积分解锁；左侧树节点<strong>展示题量</strong>；选中「我的题库」时，在平台/我的页签下方左对齐展示<strong>共 N 题</strong>（N=用户个人题库题目总数，含上传、录题等入库题目）。</p>
        <p>题库搜索框内容需持久化到本地（如 <code>localStorage</code>），用户切换到「组题工具」等其它左侧页签再返回「题库选题」或「我的题库」时，搜索词与筛选结果状态保持一致；切换学段学科或顶栏「题库选题 / 我的题库」时，分别恢复该维度下上次保存的搜索词。</p>
      </section>
      <section class="prd-section">
        <h3>6. 空状态与提示文案</h3>
