  ...
  const documents = {
    ...
    workbench: `
      ...
      <section class="prd-section">
        ...
        <p>飞象题库为新增页面。左侧负责找题、预览题源与 AI 组题过程，右侧为当前题单画布。左侧顶部为<strong>来源页签栏</strong>：学段学科下拉，固定「题库选题」「快捷组题」「我的题库」，以及由快捷组题衍生的<strong>可关闭动态页签</strong>（规则见 §2）。</p>
        <ul><li>题库选题：下分「教材章节」「知识点」。「教材章节」与飞象老师「作业」一致：选学段学科、教材版本后按章节树选题，不另述交互。「知识点」按学科知识树浏览，不选教材版本。</li><li>快捷组题：AI录题、我的题单、AI组题。</li><li>我的题库：个人入库题目，支持知识点/题型/难度/搜索；树节点展示题量。</li><li>右侧画布：展示已选题目、答案解析及作答区；须点击<strong>保存</strong>才写入「我的题单」。</li></ul>
      </section>
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>类型</th><th>示例</th><th>关闭</th></tr></thead><tbody>
          <tr><td>固定页签</td><td>题库选题、快捷组题、我的题库</td><td><strong>不可</strong>关闭；始终位于栏最左侧（「飞象题库」标识与学段学科下拉之后）。</td></tr>
      ...
      <section class="prd-section">
        ...
        <p>题库搜索框内容需持久化到本地（如 <code>localStorage</code>），用户切换到「快捷组题」等其它左侧页签再返回「题库选题」或「我的题库」时，搜索词与筛选结果状态保持一致；切换学段学科或顶栏「题库选题 / 我的题库」时，分别恢复该维度下上次保存的搜索词。</p>
      </section>
      <section class="prd-section">
        <h3>6. 空状态与提示文案</h3>
      ...
      <section class="prd-section">
        <h3>8. 快捷组题</h3>
        <p>页面标题下不展示总括性说明，直接呈现三张入口卡片。「我的题单」卡片说明为：<strong>选择已保存的题单，支持整份或逐题加入当前题单</strong>。</p>
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>入口</th><th>产品定义</th><th>完成态</th></tr></thead><tbody>
          <tr><td>AI录题</td><td>上传区主文案为「点击选择文件，或拖拽文件到此处」；说明为「支持批量上传 PNG、JPG 图片，也支持 PDF、DOCX；单个文件不超过 20 MB」。AI 识别、拆题、提取答案并自动打标，生成个人题库。</td><td>每份上传任务独立进入「AI录题进度」；<strong>解析成功后</strong>题目以<strong>结构化题目</strong>写入「我的题库」（题型、知识点、难度、分值、选项/答案结构齐全，见 §9.1），可逐题或全部加入当前题单。</td></tr>
        ...
        <h4>8.3 我的题单</h4>
        <p>入口：<strong>快捷组题 → 我的题单</strong>。页面说明统一为<strong>「这里展示你已保存的题单，可整份或逐题加入当前题单。」</strong>；右侧当前题单保持可见。</p>
        <h4>数据来源与列表</h4>
