    ...
    workbench: `
      ...
      <section class="prd-section">
        ...
        <div class="prd-table-wrap"><table class="prd-table"><thead><tr><th>步骤</th><th>交互</th><th>规则</th></tr></thead><tbody>
          ...
          <tr><td>3.1 生成中 · 思考过程</td><td>候选未返回前，在改编区内展示<strong>思考过程</strong>（分步文案 + loading）；提示「生成完成后自动收起」。</td><td>仅生成中可见；<strong>候选题就绪后思考过程自动隐藏</strong>，不得与结果列表并存。</td></tr>
          <tr><td>4. 候选结果</td><td>卡片内列表展示 2–3 道候选变式；每道<strong>不展示任何标签</strong>（无题型/难度/知识点角标），仅保留「AI 改编 N」序号与题干；右侧操作为<strong>「＋」图标</strong>（title/aria 仍为「选用题目」）。</td><td>点击「＋」后才加入右侧画布或首页已选；未选用关闭面板不影响原题。</td></tr>
