(() => {
  ...
  function renderTree(){tree.innerHTML=groups.map(([name,children])=>`<section class="tree-group"><button class="tree-parent" data-knowledge=""><span>${name}</span><em>${children.length}</em></button>${children.map(item=>`<button class="tree-child ${knowledge===item?'active':''}" data-knowledge="${esc(item)}"><span>${esc(item)}</span><em>${questions.filter(q=>q.knowledge===item).length}</em></button>`).join('')}</section>`).join('')}
  function filtered(){const type=document.querySelector('#typeFilter').value,diff=document.querySelector('#difficultyFilter').value,query=document.querySelector('#questionSearch').value.trim().toLowerCase();return questions.filter(q=>(!knowledge||q.knowledge===knowledge)&&(type==='全部题型'||q.type===type)&&(diff==='全部难度'||q.difficulty===diff)&&(!query||`${q.text}${q.knowledge}`.toLowerCase().includes(query)))}
  function answerFor(q){return ({b1:'2又1/3小时',b2:'24、20；40、12',b3:'C. 线段',b4:'B. 线段',b5:'C. 39',b6:'2个',b7:'425米/分',b8:'A. 180°',b9:'2/9立方分米',b10:'D'})[q.id]||q.answer||'请查看题库答案。'}
  function analysisFor(q){return ({b1:'先计算两种无人机每小时完成的工作量，再用剩余工作量除以效率和。',b2:'沿不同方向对折时，对折方向的边长减半，另一边不变。',b3:'线段有两个端点，符合“有始有终”的含义。',b4:'延长后仍有两个确定端点，所以仍是线段。',b5:'用9999除以256并取不超过结果的最大整数。',b6:'将各因数分解质因数，统计2和5能够配成多少组10。',b7:'先求乙领先的路程，再利用追及路程除以追及时间求速度差。',b8:'分针30分钟旋转半圈，即180°。',b9:'长方体体积等于底面积乘高。',b10:'两次对折的方向不同，折痕关系也会不同。'})[q.id]||`围绕“${q.knowledge}”提取条件，选择对应公式或关系进行推理。`}
  function render(){const rows=filtered();list.innerHTML=rows.length?rows.map((q)=>`<article class="question-card ${selected.has(q.id)?'selected':''} ${answersOpen.has(q.id)||adaptOpen.has(q.id)?'panel-open':''}" data-id="${q.id}"><div class="question-meta"><span>${q.type}</span><span>${q.difficulty}</span><span>${q.knowledge}</span></div><p>${esc(q.text)}</p>${q.options?`<div class="question-options">${q.options.map(esc).map(x=>`<span>${x}</span>`).join('')}</div>`:''}<div class="question-actions"><button type="button" data-answer="${q.id}" title="${answersOpen.has(q.id)?'收起答案':'显示答案和解析'}" aria-label="${answersOpen.has(q.id)?'收起答案':'显示答案和解析'}">答</button><button type="button" class="adapt" data-adapt="${q.id}" title="AI改编" aria-label="AI改编">✧</button><button type="button" class="add ${selected.has(q.id)?'added':''}" data-add="${q.id}" title="${selected.has(q.id)?'从已选题目移除':'添加题目'}" aria-label="${selected.has(q.id)?'从已选题目移除':'添加题目'}">${selected.has(q.id)?'✓':'＋'}</button></div>${answersOpen.has(q.id)?`<div class="answer-panel"><p><b>答案</b>${esc(answerFor(q))}</p><p><b>解析</b>${esc(analysisFor(q))}</p></div>`:''}${adaptOpen.has(q.id)?`<div class="adapt-panel"><b>AI改编</b><span>保持考点，调整数据与情境</span><button type="button" data-run-adapt="${q.id}">生成变式题</button></div>`:''}</article>`).join(''):'<div class="empty">没有找到符合条件的题目</div>'}
  tree.addEventListener('click',e=>{const button=e.target.closest('[data-knowledge]');if(!button)return;knowledge=button.dataset.knowledge;renderTree();render()})
  list.addEventListener('click',e=>{
    ...
    if(add){const q=questions.find(item=>item.id===add.dataset.add);selected.has(q.id)?selected.delete(q.id):selected.set(q.id,q);render();notify();return}
    ...
    if(run){const q=questions.find(item=>item.id===run.dataset.runAdapt);q.text=`${q.text}（AI变式：已调整题目数据）`;adaptOpen.delete(q.id);render()}
  })
  ;['typeFilter','difficultyFilter','questionSearch'].forEach(id=>document.querySelector(`#${id}`).addEventListener(id==='questionSearch'?'input':'change',render))
  document.querySelectorAll('[data-source]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-source]').forEach(x=>x.classList.toggle('active',x===button));render()}))
