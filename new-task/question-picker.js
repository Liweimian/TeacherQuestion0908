(() => {
  ...
  function render(){
    ...
    const pageInfo=rows.length?`<div class="picker-page-info"><span>${currentSource==='official'?'平台题库':'我的题库'}</span><b>本页 ${rows.length} 道题</b>${currentSource==='official'?`<em>第 ${currentPage} / ${totalPages} 页</em>`:''}</div>`:''
